	...
	<table class="table">
		<c:forEach items="${list}" var="v">
			<tr>
				...
				<td class="col-6">${v.boardContents}</td>
				<td class="col-2 small text-muted">${v.boardDate}</td>
				<c:if test="${not empty user and v.username eq user.username}">
				  <td class="col-3">
				    <button type="button" class="btn btn-warning btn-sm py-0 px-2">수정</button>
				    <button type="button" class="btn btn-danger btn-sm py-0 px-2">삭제</button>
				  </td>
				</c:if>
			</tr>
		</c:forEach>
